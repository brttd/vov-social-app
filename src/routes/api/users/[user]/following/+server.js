...
export async function GET({ locals, params, url }) {
	...
	const query = db('users')
		.leftJoin('user_follows', 'user_follows.follows_user_id', 'users.id')
		.where({
			'user_follows.user_id': params.user
		});

	const data = await query.select([
		'id',
		'username', // TODO: redo this to use the actual KNEX query builder
